...
class CustomDocumentAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if dq is None:
            ...
            dq.save()
        if ds is None:
            ds = DocumentSummary(content='', prompt_text='concise summary', company = cur_user, document_id= str(obj.id))
            ds.save()
        if dkp is None:
            dkp = DocumentKeyPoints(content='', prompt_text='concise outline in numeric order list', company = cur_user, document_id= str(obj.id))
            dkp.save()
