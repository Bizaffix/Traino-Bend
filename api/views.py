...
from api.serializers import (
    ...
    DepartmentRUDSerializers,
    CompanyTeamSerializer,
    UserCreateSerializer,
    DocumentSerializer,
    ReadOnlyDocumentSerializer,
    DocumentSummarySerializer,
    ReadOnlyDocumentSummarySerializer,
    DocumentKeypointsSerializer,
    ReadOnlyDocumentKeypointsSerializer,
)
...
def assignDocumentToUser(user, doc, department):
    try:
        das = DocumentTeam.objects.get(
            user_id=user.id, document_id=doc.id, department_id=department.id
        )
...
def generateDocumentSummary(request):
    ...
    document_summary = (
        "Oops! Summary not generated please try with some other document."
    )
    ...
    if document.file.path is not None:
        try:
            ...
            if error.code == "insufficient_quota":
                ...
                # print(error)
                raise serializers.ValidationError(
                    "Your document content is too large, Please try with some other document."
                )
...
def generateDocumentKeypoints(keypoint_id, document_id, prompt_text):
    document_keypoints = (
        "Oops! Keypoints not generated please try with some other document."
    )
...
class DepartmentCreateApiview(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        try:
            company_name = Departments.objects.get(
                name=name, company=company_id_from_request, is_active=True
            )
            if company_name:
                raise serializers.ValidationError(
                    {
        ...
        if self.request.user.role == "Admin":
            try:
                admin_user = AdminUser.objects.get(
                    admin=self.request.user, is_active=True
                )
            except AdminUser.DoesNotExist:
                raise serializers.ValidationError(
                    {
                        "Error": "Your account is deleted as Admin by Super Admin. You can no Longer access this feature"
                    }
                )
            ...
            if str(admin_user.company.id) != company_id_from_request:
                raise serializers.ValidationError(
                    {
                        "Access Denied": "You are not allowed to create departments in this company."
                    }
                )
...
class DepartmentRetrieveApiView(RetrieveAPIView, UpdateAPIView, DestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if str(instance.company.id) == str(admin.company.id):
            if admin.is_active == True:
                ...
                if new_name and new_name != instance.name:
                    if Departments.objects.filter(
                        name=new_name, is_active=True
                    ).exists():
                        raise serializers.ValidationError(
                            {
                                "Error": "A department with this name already exists and is active."
                            }
                        )
            ...
            return Response(
                {
                    "Account Error": "Your Profile is restricted . You are not allowed to perform this action"
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        return Response(
            {
                "Account Error": "Your Profile is Not Authorized for this request as you are requesting data for unknown company department."
            },
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if str(instance.company.id) == str(admin.company.id):
            if admin.is_active == True:
                ...
                department_documents = DepartmentsDocuments.objects.filter(
                    id=instance.id
                )
                ...
                return Response(
                    {
                        "Delete Status": "Successfully deleted the department.",
                        "Department_id": instance.id,
                    },
                    status=status.HTTP_202_ACCEPTED,
                )
            return Response(
                {"Account Error": "Account did not found"},
                status=status.HTTP_404_NOT_FOUND,
            )
        return Response(
            {
    ...
    def get_queryset(self):
        ...
        if user.role == "Admin":
            ...
            if str(company_id) != str(admin.company.id):
                raise serializers.ValidationError(
                    {
                        "Permission Denied": "You are not allowed to view departments of this company"
                    }
                )
            if not admin.is_active:
                raise serializers.ValidationError(
                    {
                        "Permission Denied": "Your account is restricted. You cannot perform this task"
                    }
                )
            if company_id:
            ...
            if not user_id:
                raise serializers.ValidationError(
                    {"user_id": "This query parameter is required for users."}
                )

            try:
                user_teams = CompaniesTeam.objects.filter(members=user, is_active=True)
                user_departments = Departments.objects.filter(
                    users__in=user_teams, is_active=True
                )

                if user_id:
                    user_teams_filtered = CompaniesTeam.objects.filter(
                        id=user_id, members=user, is_active=True
                    )
                    if user_teams_filtered.exists():
                        user_departments = user_departments.filter(
                            users__in=user_teams_filtered
                        ).distinct()
                    else:
                        raise serializers.ValidationError(
                            {
                                "Permission Denied": "You are not allowed to view departments for this user_id."
                            }
                        )
                ...
                raise serializers.ValidationError(
                    {"Permission Denied": "You are not allowed to view departments."},
...
class CompanyTeamModelViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.validated_data["department"] = Departments.objects.get(
            pk=request.data["department"]
        )
        self.perform_create(serializer)

        company_documents = UserDocuments.objects.filter(
            company_id=self.request.user.id, department=int(request.data["department"])
        )
        for company_document in company_documents:
            assignDocumentToUser(
                CustomUser.objects.get(pk=int(serializer.data["id"])),
                company_document,
                Departments.objects.get(pk=request.data["department"]),
            )
...
class AddUserToDepartmentView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request_user.role == "Admin" and admin.is_active:
            ...
            if not user_ids or not department_ids:
                if not user_ids:
                    ...
                    return Response(
                        {"detail": "Department IDs are required."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            users_not_found = []
        ...
        return Response(
            {"detail": "Your account is restricted. You cannot perform this task."},
            status=status.HTTP_401_UNAUTHORIZED,
        )
...
class CreateSummaryApiView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "No Document Found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if user.role == "Super Admin":
            ...
            if admin.is_active == True:
                if not document.departments.filter(company=admin.company).exists():
                    return Response(
                        {
                            "Access Denied": "You are not allowed to view the summary of this document"
                        },
                        status=status.HTTP_401_UNAUTHORIZED,
                    )
            else:
            ...
            if member.is_active == True:
                user_departments = Departments.objects.filter(
                    users=member, is_active=True
                )
                if not user_departments.exists():
                    return Response(
                        {
                            "No Association": "You are no longer associated with this department"
                        },
            ...
            if not user_departments.filter(
                id__in=document.departments.values_list("id", flat=True)
            ).exists():
                return Response(
                    {
                        "Access Denied": "You are not allowed to view the summary of this department"
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
        try:
            ...
            if not summary:
                return Response(
                    {"Not Found": "No Summary Found against this document."},
                    status=status.HTTP_404_NOT_FOUND,
                )
            if user.role == "User":
                return Response(
                    {"id": summary.id, "summary": summary.summary},
    ...
    def post(self, request):
        if request.user.role == "Admin":
            ...
            try:
                document = DepartmentsDocuments.objects.get(
            ...
            if user.role == "Admin":
                ...
                if admin.is_active == True:
                    if not document.departments.filter(company=admin.company).exists():
                        return Response(
                            {
                                "Access Denied": "You are not allowed to create the summary of this document"
                            },
                            status=status.HTTP_401_UNAUTHORIZED,
                        )
                else:
            ...
            if not document.file:
                return Response(
                    {"Not Found": "No File is associated"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            summary_request = DocumentSummary.objects.filter(
                document=document, added_by=user
            ).count()
            if summary_request == 5:
                return Response(
                    {
                        "error": "Summary creation limit reached. No more than 5 requests allowed for this document."
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            created_summary = DocumentSummary.objects.create(
                summary=summary, document=document, prompt=_, added_by=self.request.user
            )
            ...
            return Response(
                {
                    "id": created_summary.id,
                    "summary": f"{created_summary.summary}",
                    "prompt": f"{created_summary.prompt}",
                },
                status=status.HTTP_200_OK,
            )
        return Response(
            {"Access Denied": "You Are not Allowed to create summary"},
            status=status.HTTP_401_UNAUTHORIZED,
        )
...
class CreateKeypointsApiView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            if user.role == "User":
                return Response(
                    {"id": keypoints.id, "keypoints": keypoints.keypoints},
                    status=status.HTTP_200_OK,
                )
            return Response(
                {
    ...
    def post(self, request):
        if request.user.role == "Admin":
            ...
            try:
                print("DLOOO", document_id)
                document = DepartmentsDocuments.objects.get(
                    id=document_id, is_active=True
                )
                # print("DLOOO", document)
            ...
            keypoint_count = DocumentKeyPoints.objects.filter(
                document=document, added_by=user
            ).count()
            ...
            if keypoint_count == 5:
                return Response(
                    {
                        "error": "Keypoints creation limit reached. No more than 5 requests allowed for this document."
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            return Response(
                {
                    "id": created_keypoint.id,
                    "keypoints": f"{created_keypoint.keypoints}",
                    "prompt": f"{created_keypoint.prompt}",
                },
                status=status.HTTP_200_OK,
            )
        return Response(
            {"Access Denied": "You Are not Allowed to create keypoints"},
            status=status.HTTP_401_UNAUTHORIZED,
        )
...
class QuizResultsApiView(APIView):
    def get(self, request):
        if request.user.role == "User":
            return Response(
                {"Access Denied": "You are not allowed to view the data"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        else:
            quiz_id = request.query_params.get(
                "quiz_id"
            )  # Retrieve quiz_id from query parameters
...
class CreateQuizessApiView(APIView):
        ...
        if user.role == "Super Admin":
            ...
            if admin.is_active:
                if document.departments.filter(company=admin.company).exists():
                    ...
                    return Response(
                        {
                            "Access Denied": "You are not allowed to view quizzes for this document"
                        },
                        status=status.HTTP_401_UNAUTHORIZED,
                    )
            else:
            ...
            if member and member.is_active:
                assigned_documents = DepartmentsDocuments.objects.filter(
                    departments__users=member, is_active=True
                )
                if document in assigned_documents:
                    quizzes = DocumentQuiz.objects.filter(
                        document=document, upload=True
                    )
                else:
                    return Response(
                        {
    ...
    def post(self, request):
        if request.user.role == "Admin":
            ...
            if user.role == "Admin":
                ...
                if admin.is_active == True:
                    if not document.departments.filter(company=admin.company).exists():
                        return Response(
                            {
                                "Access Denied": "You are not allowed to create the Quiz of this document"
                            },
                            status=status.HTTP_401_UNAUTHORIZED,
                        )
                else:
            ...
            quiz_count = DocumentQuiz.objects.filter(
                document=document, added_by=user
            ).count()
            if quiz_count >= 10:
                return Response(
                    {
                        "error": "Quiz creation limit reached. No more than 10 quizzes allowed for this document."
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            if len(questions_data) - 1 > 10:
                return Response(
                    {"error": "Each quiz can have a maximum of 10 questions."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"id": created_quiz.id, "quiz": f"{created_quiz.quiz}"},
                status=status.HTTP_200_OK,
            )
        return Response(
            {"Access Denied": "You Are not Allowed to create quiz"},
            status=status.HTTP_401_UNAUTHORIZED,
        )
        ...
        try:
            if request.user.role == "Admin":
                ...
                try:
                    ...
                    return Response(
                        {"error": "No Quiz Found"}, status=status.HTTP_404_NOT_FOUND
                    )
                ...
                if str(admin.company.id) != str(
                    quiz.document.departments.first().company.id
                ):
                    return Response(
                        {"Access Denied": "You are not allowed to delete this quiz"},
                        status=status.HTTP_401_UNAUTHORIZED,
                    )

                quiz.delete()
                logger.info(
                    f"Quiz with id {quiz.id} deleted successfully by {request.user.id}"
                )
                return Response(
                    {"message": "Quiz deleted successfully", "id": quiz_id},
                    status=status.HTTP_204_NO_CONTENT,
                )
            else:
                return Response(
                    {"Access Denied": "You are not allowed to delete quizzes"},
                    status=status.HTTP_401_UNAUTHORIZED,
                )
            ...
            return Response(
                {"Access Denied": str(e)}, status=status.HTTP_401_UNAUTHORIZED
            )
        except Exception as e:
...
class DeleteQuizApiView(DestroyAPIView):
    ...
    def destroy(self):
        ...
        quiz.delete()
        return Response(
            {"Delete Message": "Quiz Deleted Successfully", "id": id.id},
            status=status.HTTP_200_OK,
        )
...
class UploadQuiz(APIView):
    ...
    def post(self, request):
        ...
        if quiz:
            ...
            quiz.save()
            return Response(
                {"message": "Quiz is Successfully uploaded or updated", "id": quiz.id},
                status=status.HTTP_202_ACCEPTED,
            )
        return Response(
            {"Not Found": "No quiz found"}, status=status.HTTP_404_NOT_FOUND
        )
...
class EditQuizes(APIView):
    ...
    def update_question(self, request, question_id, partial):
        ...
        if str(admin.company.id) != str(
            question.quiz.document.departments.first().company.id
        ):
            return Response(
                {"Access Denied": "You are not allowed to edit this question"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if (
            ...
            del request.data[
                "options"
            ]  # Remove options from request data to avoid conflict
        elif options:
            return Response(
                {"error": "Invalid options format"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def delete(self, request, question_id):
        ...
        if str(admin.company.id) != str(
            ...
            return Response(
                {"Access Denied": "You are not allowed to delete this question"},
                status=status.HTTP_403_FORBIDDEN,
            )
        q_id = question.id
        question.delete()
        return Response(
            {"message": "Question deleted successfully", "id": q_id},
            status=status.HTTP_200_OK,
        )
...
class SubmitQuizView(APIView):
    ...
    def post(self, request):
        if request.user.role == "Admin" or request.user.role == "Super Admin":
            return Response(
                {"Access Denied": "You are not authorized for this request"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
            ...
            if not user_departments.filter(id=document_department.id).exists():
                return Response(
                    {
                        "Access Denied": "You are not allowed to submit quiz for this document"
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            for answer_data in answers:
                ...
                try:
                    ...
                    return Response(
                        {
                            "error": f"Question with id {question_id} not found in the quiz"
                        },
                        status=status.HTTP_404_NOT_FOUND,
                    )
            ...
            return Response(
                {"Score": score, "Status": status_test}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"Access Denied": "Unauthorized Access Requested"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
...
class DocumentSummaryModelViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if user.role == "Admin":
            try:
                ...
                prompt_text = self.request.data.get(
                ...
                if str(requested_company_id) != str(company):
                    raise serializers.ValidationError(
                        "You can only create summaries for your own company."
                    )
                ...
                truncated_content = self.truncate_content(
                    document_content, max_input_tokens
                )
                ...
                raise serializers.ValidationError(
                    "Document not found or is not active."
                )
        else:
            raise serializers.ValidationError(
                "Only Admins can create document summaries."
            )
    ...
    def perform_update(self, serializer):
        ...
        if user.role == "Admin" and instance.added_by == user:
            ...
            raise serializers.ValidationError(
                "You do not have permission to update this summary."
            )
    ...
    def perform_destroy(self, instance):
        ...
        if user.role == "Admin" and instance.added_by == user:
            ...
            instance.delete()
            return Response(
                {"Response": "Successfully Deleted the Summary.", "id": obj_id}
            )
        else:
            raise serializers.ValidationError(
