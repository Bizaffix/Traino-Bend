...
def call_model(prompt, content, model, system_mssg):
    api_key = os.getenv("DEEPSEEK_API_KEY") if model == "deepseek-chat" else os.getenv("OPENAI_API_KEY")
    base_url = "https://api.deepseek.com" if model == "deepseek-chat" else "https://api.openai.com"
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": system_mssg},
                {"role": "user", "content": f"{prompt}{content}"},
            ],
            stream=False,
            temperature=0.5,
...
def generate_summary_from_gpt(content, prompt=None):
    ...
    try:
        return call_model(prompt, content, "deepseek-chat", "You are a professional assistant specializing in summarizing content.")
    except:
        return call_model(prompt, content, "gpt-3.5-turbo", "You are a professional assistant specializing in summarizing content.")

# Function to generate key points
def generate_keypoints_from_gpt(content, prompt=None):
    prompt = prompt or (
        "Extract key points from the following content. Present them as bullet points "
        "that are concise, relevant, and unique. Focus on capturing critical ideas, "
    ...
    try:
        return call_model(prompt, content, "deepseek-chat", "You are a professional assistant specializing in generating key points.")
    except:
        return call_model(prompt, content, "gpt-3.5-turbo", "You are a professional assistant specializing in generating key points.")

# Function to generate quizzes
def generate_quizzes_from_gpt(content, max_questions=10, min_questions=5):
    prompt = (
        f"You are a teacher creating quizzes. Provide a quiz with a maximum of {max_questions} "
        f"questions and a minimum of {min_questions} questions, each containing 4 options as MCQs. "
        "Indicate the correct option by labeling it A, B, C, or D and answer as 'Correct Answer'. "
    ...
    try:
        return call_model(prompt, content, "deepseek-chat", "You are a professional quiz generator.")
    except:
        return call_model(prompt, content, "gpt-3.5-turbo", "You are a professional quiz generator.")
